...
angular.module('app').controller('CheckoutPbsCtrl', ['$scope','$ngBootbox' ,'$http','socket_url','$stateParams','$timeout','$state','toaster','$filter','$cookieStore',
    function($scope,$ngBootbox,$http ,socket_url,$stateParams,$timeout,$state,toaster,$filter,$cookieStore ) {
        ...
        if($stateParams.code){
            ...
            $scope.edit_pbs_id = $stateParams.code ;
        }
        ...
        $scope.detail_click_mod = function(data){
            setTimeout(function () {
                ...
                $ngBootbox.customDialog({
                    ...
                    templateUrl: 'tpl/custom/pbs/detail.html',
        ...
        $scope.total_mua = {};
        $scope.total_chiphi = {};
        $scope.get_total_again = function(pbs){
            $scope.total_mua[pbs._id] = 0 ;
            $scope.total_chiphi[pbs._id] = 0 ;
            angular.forEach(pbs.services, function(service) {
                $http.get('/api/check_service/' + service.code)
                    .success(function(data) {
                        if(data != null &&  data.price_mua != null){
                            if(service.nc){
                                $scope.total_mua[pbs._id] = $scope.total_mua[pbs._id] +  data.price_mua*service.nc.nc_qty*service.qty ;
                                $scope.total_chiphi[pbs._id] = $scope.total_chiphi[pbs._id] +  data.price_chiphi*service.nc.nc_qty*service.qty ;
                            }else{
                                $scope.total_mua[pbs._id] = $scope.total_mua[pbs._id] + data.price_mua*service.qty ;
                                $scope.total_chiphi[pbs._id] = $scope.total_chiphi[pbs._id] + data.price_chiphi*service.qty ;
                            }
            ...
            setTimeout(function(){
                $http.post('/api/update_pbss/' ,{ _id : pbs._id,total_mua :  $scope.total_mua[pbs._id] ,total_chiphi :  $scope.total_chiphi[pbs._id]})
                    .success(function(data) {
        ...
        //-------------
        $scope.del = function(pbs) {
            $http.post('/api/logs', {
                active : "Xóa 1 Phiếu Bán Hàng ( Tổng Thanh Toán : "+ $filter('number')(pbs.total ,0) +" )",
        ...
        $scope.edit_pbs = function(id){
            $scope.edit_ser = false ;
            if( $scope.edit_pbs_id == null){
                $scope.edit_pbs_id = id;
            }else{
                if($scope.edit_pbs_id != id){
                    $scope.edit_pbs_id = id;
                }else if($scope.edit_pbs_id != null){
                    $scope.edit_pbs_id = null;
                }
            }
        };
        ...
        $scope.printDiv = function(divName , pbs) {
            var contents = $("#"+divName).html();
            var frame1 = $('<iframe />');
            frame1[0].name = "frame1";
            frame1.css({ "position": "absolute", "top": "-1000000px" });
            ...
            frameDoc.document.write('<div class="info-bill-mod">'+pbs.code+'</div>');
            ...
            frameDoc.document.write('<div class="bot-thanks-bill">Thu Ngân : '+pbs.user.name+'</div>');
            if(pbs.coupon){
                frameDoc.document.write('<div class="bot-thanks-bill">Mã Giảm Giá : '+pbs.coupon+'</div>');
            }
            frameDoc.document.write('<div class="bot-date-bill">Ngày :'+$filter('date')(pbs.date, 'dd/MM/yyyy')+' Giờ '+
                $filter('date')(pbs.date, 'HH:mm')+' </div>');
            /*
