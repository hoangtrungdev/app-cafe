...
angular.module('app').controller('ReturnCtrl', ['$scope','$ngBootbox' ,'$http','socket_url','$stateParams','$timeout','$state','toaster','$filter','$cookieStore',
    function($scope,$ngBootbox,$http ,socket_url,$stateParams,$timeout,$state,toaster,$filter,$cookieStore ) {
        ...
        if($stateParams.code){
            ...
            $scope.edit_return_id = $stateParams.code ;
        }
        ...
        //-------------
        $scope.del = function(ret) {
            $http.post('/api/logs', {
                active : "Xóa 1 Phiếu Trả Hàng ( Tổng Thanh Toán : "+ $filter('number')(ret.total ,0) +" )",
            ...
            $http.delete('/api/returns/' + ret._id)
                .success(function(data) {
        ...
        //-------------------------------------
        $scope.edit_return = function(id){
            $scope.edit_ser = false ;
            if( $scope.edit_return_id == null){
                $scope.edit_return_id = id;
            }else{
                if($scope.edit_return_id != id){
                    $scope.edit_return_id = id;
                }else if($scope.edit_return_id != null){
                    $scope.edit_return_id = null;
                }
            }
        };
        ...
        $scope.printDiv = function(divName , checkout) {
            var contents = $("#"+divName).html();
            var frame1 = $('<iframe />');
            frame1[0].name = "frame1";
            frame1.css({ "position": "absolute", "top": "-1000000px" });
            ...
            frameDoc.document.write('<div class="info-bill-mod">'+checkout.code+'</div>');
            ...
            frameDoc.document.write('<div class="bot-thanks-bill">Thu Ngân : '+checkout.user.name+'</div>');
            if(checkout.coupon){
                frameDoc.document.write('<div class="bot-thanks-bill">Mã Giảm Giá : '+checkout.coupon+'</div>');
            }
            frameDoc.document.write('<div class="bot-date-bill">Ngày :'+$filter('date')(checkout.date, 'dd/MM/yyyy')+' Giờ '+
                $filter('date')(checkout.date, 'HH:mm')+' </div>');
            /*
